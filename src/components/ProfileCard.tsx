...
export function ProfileCard({ profile }: { profile: Profile }) {
  ...
  return (
    <article className="flex h-full flex-col rounded-md border border-border bg-background p-4 transition-colors hover:border-accent">
      <div className="flex items-center gap-3">
        <Avatar handle={profile.github} name={profile.name} size={48} />
        <div className="min-w-0">
          <Link
            href={`/p/${profile.github.toLowerCase()}`}
            className="block truncate text-base font-semibold hover:text-accent hover:underline"
          >
      ...
      {profile.role && <p className="mt-3 text-xs font-medium uppercase tracking-wide text-muted">{profile.role}</p>}
      <p className="mt-1 flex-1 text-sm">{profile.bio}</p>
      {profile.building && (
